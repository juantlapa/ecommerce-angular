    ...
    if (type === "credit_card" || type === "debit_card") {
      if (!cardNumber || !cardHolderName || !expiryDate) {
        return res.status(400).json({
          error:
            "Card number, card holder name, and expiry date are required for card payments",
        });
      }
      ...
      if (!paypalEmail) {
        return res
          .status(400)
          .json({ error: "PayPal email is required for PayPal payments" });
      }
...
async function setDefaultPaymentMethod(req, res, next) {
  try {
    ...
    if (!paymentMethod.isActive) {
      return res
        .status(400)
        .json({ message: "Cannot set inactive payment method as default" });
    }
...
async function getDefaultPaymentMethod(req, res, next) {
  try {
    ...
    if (!defaultPaymentMethod) {
      return res
        .status(404)
        .json({ message: "No default payment method found for this user" });
    }
